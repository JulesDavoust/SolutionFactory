...
export default {
    ...
    created() {
        this.searchTag= null
        this.arrayEbooks = []
        this.searchTag = this.$route.params.searchTag;
    ...
    watch: {
        ...
        function (newParams, oldParams) {
            if (newParams.searchTag !== oldParams.searchTag) {
            this.searchTag = newParams.searchTag;
            this.fetchData();
            }
        }
    },
  ...
  methods: {
    fetchData() {
      this.arrayEbooks = [];
      console.log(this.searchTag);
      fetch(`http://localhost:8080/api/tags/tagName/${this.searchTag}`)
        .then(response => response.json())
        .then(datIdTag => {
          this.IdTag = datIdTag;
          console.log(this.IdTag.idTag);
          fetch(`http://localhost:8080/api/ebooksByTags/${this.IdTag.idTag}`)
            .then(response => response.json())
            .then(dataEbooks => {
              this.Ebooks = dataEbooks;
              console.log(this.Ebooks);
              for (let i = 0; i < this.Ebooks.length; i++) {
                fetch(`http://localhost:8080/api/ebooks/oneEbook/${this.Ebooks[i].idEbook}`)
                  .then(response => response.json())
                  .then(dataEbooks => {
                    this.arrayEbooks.push(dataEbooks);
                    console.log("ArrayEbooks ", this.arrayEbooks);

                    // Récupérer les informations de l'auteur pour chaque ebook
                    for (let j = 0; j < this.arrayEbooks.length; j++) {
                      fetch(`http://localhost:8080/api/authors/oneAuthor/${this.arrayEbooks[j].authorID}`)
                        .then(response => response.json())
                        .then(data => {
                          console.log("test ", data);
                          // Mettre à jour le dictionnaire 'authors' avec les informations de l'auteur
...
.disabled-button {
  ...
  /* Autres styles pour les boutons désactivés */
}
